...
class AlorPy:
    ...
    def get_jwt_token(self):  # https://alor.dev/docs/api/http/jwt-token
        ...
        if self.jwt_token is None or now - self.jwt_token_issued > self.jwt_token_ttl:  # Если токен JWT не был выдан или был просрочен
            max_attempts = int(os.environ.get("ALOR_JWT_MAX_ATTEMPTS", "5"))
            backoff_seconds = int(os.environ.get("ALOR_JWT_BACKOFF_SECONDS", "5"))
            timeout_seconds = int(os.environ.get("ALOR_JWT_TIMEOUT_SECONDS", "10"))
            response = None
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    self.on_error(f'Ошибка запроса JWT токена: {e}')
                if attempt < max_attempts:
                    sleep(backoff_seconds)
            if response is None:
                self.jwt_token = None
                self.jwt_token_decoded = None
    ...
    def get_symbol(self, exchange, symbol, instrument_group=None, format='Simple'):  # https://alor.dev/docs/api/http/md-v-2-securities-exchange-symbol-get
        ...
        # Кол-во десятичных знаков получаем из шага цены.
        # Иногда minstep может отсутствовать/быть None — тогда не падаем.
        if not result:
            return None

        minstep = result.get('minstep')
        if not minstep:
            result['decimals'] = 0
        else:
            result['decimals'] = int(log10(1 / minstep) + 0.99)
        return result
